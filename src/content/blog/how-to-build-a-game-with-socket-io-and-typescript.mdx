...
pubDate: '2022-02-14'
tags: Workshop,Realtime,React,Node.js
---
...
![computer guy finally done](/content/how-to-build-a-game-with-socket-io-and-typescript/computer_guy_finally_done.gif)
...
```graphviz
digraph CreateRoomFlow {
  A [label="The client emits an event that it wants to\ncreate a room\nPLAYER_CREATE_ROOM_EVENT"];
  B [label="The server creates the room\n then emits another event that the room\nhas been created\nROOM_CREATED_EVENT"];
  C [label="Once the client receives the ROOM_CREATED_EVENT\n it redirects the user\ninto a new page where the game can be played"];
...
```graphviz
digraph MoveFlow {
  A [label="The client emits an event that\n the player wants to make a move\nPLAYER_MOVED_EVENT"];
  B [label="The server handles that and if it's successful,\n the server broadcasts the new state\n as another event GAME_STATE_CHANGED"];
  C [label="Once the client receives the GAME_STATE_CHANGED\n it redraws the UI to reflect the new state."];
...
![new game](/content/how-to-build-a-game-with-socket-io-and-typescript/tt-9BaHlv.png)

Now let's build our `XO.tsx` component where the game will take place. By the end it should look like this:

![tic tac toe board](/content/how-to-build-a-game-with-socket-io-and-typescript/35GW9U0yk.png)
...
![All done!](/content/how-to-build-a-game-with-socket-io-and-typescript/all_done.gif)
